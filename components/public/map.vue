...
export default {
  ...
  mounted() {
    ...
    window.onmaploaded = () => { // onmaploaded 是一个自定义的方法，添加到window中
      ...
      window.AMap.plugin('AMap.ToolBar', () => {
        let toolbar = new window.AMap.ToolBar() // 左侧的滑动插件
        map.addControl(toolbar)
        let marker = new window.AMap.Marker({
          icon: 'https://webapi.amap.com/theme/v1.3/markers/n/mark_b.png', // 这个是图标，好像只有2中就这么写就行
          position: self.point
        })
        self.marker = marker
        marker.setMap(map)
